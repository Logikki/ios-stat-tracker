...
final class SettingsViewModel: ObservableObject {
    ...
    init(authenticationManager: AuthenticationManagerImpl) {
        ...
        activeBackendURL = active
        backendURLDraft = active
    }

    var isUsingProduction: Bool {
        activeBackendURL == Constants.API.productionURL
    }

    var isUsingLocal: Bool {
        activeBackendURL == Constants.API.localSimulatorURL
    }

    var isCustom: Bool {
        !isUsingProduction && !isUsingLocal
    }

    func useProduction() {
        applyURL(Constants.API.productionURL)
    }

    func useLocal() {
        applyURL(Constants.API.localSimulatorURL)
    }

    func applyDraft() {
        applyURL(backendURLDraft)
    }
